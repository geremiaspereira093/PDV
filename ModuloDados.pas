...
begin
	MessageDlg('Funcionário excluído com sucesso',mtConfirmation,mbOkCancel,0);
end;

procedure TDM.consultaFuncBeforeDelete(DataSet: TDataSet);
begin
	if MessageDlg('Deseja excluir o funcionário ?',mtConfirmation,[mbYes,mbNo],0) = mrNo Then
  	abort
end;
...
begin
	if MessageDlg('Deseja cancelar a alteração do produto',mtConfirmation,[mbYes,mbNo],0)= mrNo Then
    abort
end;
...
begin
	if MessageDlg('Deseja cancelar à alteração ?',mtConfirmation,[mbYes,mbNo],0) = mrNo Then
    abort;
end;

procedure TDM.DataSetEditFuncionarioAfterCancel(DataSet: TDataSet);
begin
	MessageDlg('Alteração cancelada',mtConfirmation,mbOkCancel,0);
end;

procedure TDM.DataSetEditFuncionarioAfterPost(DataSet: TDataSet);
begin
	MessageDlg('Alteração gravada com sucesso !',mtConfirmation,mbOkCancel,0);
end;
...
begin
   if MessageDlg('Deseja editar os dados do funcionário ?',mtConfirmation,[mbYes,mbNo],0) = mrNo Then
  	abort;
end;

procedure TDM.dataSetFuncionariosBeforeInsert(DataSet: TDataSet);
begin
  ShowMessage('Cadastro de funcionários');
end;
//////////////////////////produtos////////////////////////////
...
begin
 	if MessageDlg('Deseja Excluir o usuário ?', mtConfirmation,[mbYes,mbNo],0) = mrNo Then
 		abort;
end;

procedure TDM.dataSetUsuariosBeforeEdit(DataSet: TDataSet);
begin
	if MessageDlg('Deseja editar os dados do usuário ?', mtConfirmation,[mbYes,mbNo],0) = mrNo Then
    abort;
end;

procedure TDM.dataSetUsuariosBeforeInsert(DataSet: TDataSet);
begin
	ShowMessage('Cadastro de usuários !');
end;
